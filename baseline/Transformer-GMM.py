        ...
        for n_component in [2]:  # 1, 2, 3
            for n_hiddenlayer in [1]: # 1, 2
                for hidden_size in [32]: # 16, 32, 64, 128
                    ...
                    model = DeepK_GMM_Transformer(input_size, n_hiddenlayer, hidden_size, prediction_length, spatial_dim, n_component, n_head, max_len).to(device)
                    ...
                    for epoch in range(num_epochs):
                        ...
                        for train_id in range(len(train_batchxs)):
                            ...
                            path = rf"trained-model\Transformer_GMM_{encoder_length}_{n_component}_{n_hiddenlayer}_{hidden_size}_{epoch + 1}.pt"
                        ...
                        with torch.no_grad():
                            ...
                            if epoch_val_loss < best_loss:
                                ...
                                epochs_no_improve += 1
                            print(f"Epoch {epoch + 1}: Val Loss = {epoch_val_loss:.4f}, Patience = {epochs_no_improve}")
                ...
                with open(rf"trained-model\Transformer_GMM_{encoder_length}_{n_component}_{n_hiddenlayer}_{hidden_size}_{epoch + 1}.json",
                        "w") as f:
